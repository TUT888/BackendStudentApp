<?php
    ...
    empty($inputArray['idUser']) || empty($inputArray['subject']) || 
    empty($inputArray['field']) || empty($inputArray['dateTimesLearning']) || 
    empty($inputArray['learningPlaces']) || empty($inputArray['method']) || 
    empty($inputArray['tuition']) || empty($inputArray['description']) || empty($inputArray['postDate']) ) { 
    ...
    $id = update_post($id, $title, $status, $subject, $field, $dateTimesLearning, $learningPlaces, $method, $tuition, $description, $postDate);
    if ($id == 0) {
