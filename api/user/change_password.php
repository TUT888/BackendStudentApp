<?php
    header('Access-Control-Allow-Origin: *');
    header('Access-Control-Allow-Methods: POST');
    header('Access-Control-Allow-Headers: Content-Type');
    header('Content-Type: application/json; charset=UTF-8');
    require_once ($_SERVER['DOCUMENT_ROOT']."/user_db.php");

    function error_response($code, $message){
        $res = array();
        $res['code'] = $code;
        $res['message'] = utf8_encode($message);
        
        die (json_encode($res));
    }

    function success_response($data, $message){
        $res = array();
        $res['code'] = 0;
        $res['message'] = utf8_encode($message);
        $res['data'] = $data;
        
        echo json_encode($res);
    }

    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        error_response(4, 'This method is not supported');
    }

    if (empty($_POST['phoneNumber'])) {
        error_response(1, 'Please input the proper ID');
    }
    if (empty($_POST['phoneNumber'])) {
        error_response(1, 'Please input the proper Password');
    }
    
    $id = $_POST['phoneNumber'];
    $pass = $_POST['newPassword'];
           

    $result = change_password($id, $pass);
    if ($result != 0){
        error_response(5, 'An error occured when changing password');
    }
    success_response($id, 'Change password successfully');
?>